...
case ValueType::T_OBJ:\
	return value.object != nullptr ? 1 : 0;\
case ValueType::T_STRING:\
	return value.str != nullptr ? 1 : 0;\
default:\
	return 0;\
}
...
JsonValue::JsonValue(const JsonValue& rhs)
{
	...
	if (rhs.type == ValueType::T_STRING)
	{
		this->value.str = new std::string(*rhs.value.str);
	}
	else if (rhs.type == ValueType::T_OBJ)
	{
...
JsonValue::JsonValue(JsonValue&& rhs)
{
	if (this != &rhs)
	{
		this->type = rhs.type;
		this->value = rhs.value;
		rhs.type = ValueType::T_UNDEFINED;
	}
}
...
{
	this->type = ValueType::T_STRING;
	this->value.str = new std::string(std::move(val));
}

JsonValue::JsonValue(const JsonObject &val)
{
	this->type = ValueType::T_OBJ;
...
{
	this->type = ValueType::T_OBJ;
	this->value.object = new JsonObject(std::move(val));
}
...
bool JsonValue::as_bool()
{
	...
	case ValueType::T_DOUBLE:
		return !(std::abs(value.double_v) < 1e-8);
	case ValueType::T_OBJ:
		return value.object != nullptr ? true : false;
	case ValueType::T_STRING:
		return value.str != nullptr ? true : false;
	default:
		return false;
	}
}
...
std::string JsonValue::as_string()
{
	...
	{
	case ValueType::T_BOOL:
		return std::to_string(value.bool_v);
	case ValueType::T_INT8:
		return std::to_string(value.int8_v);
	case ValueType::T_INT16:
		return std::to_string(value.int16_v);
	case ValueType::T_INT32:
		return std::to_string(value.int32_v);
	case ValueType::T_INT64:
		return std::to_string(value.int64_v);
	case ValueType::T_UINT8:
		return std::to_string(value.uint8_v);
	case ValueType::T_UINT16:
		return std::to_string(value.uint16_v);
	case ValueType::T_UINT32:
		return std::to_string(value.uint32_v);
	case ValueType::T_UINT64:
		return std::to_string(value.uint64_v);
	case ValueType::T_DOUBLE:
		return std::to_string(value.double_v);
	case ValueType::T_STRING:
		return value.str != nullptr ? *value.str : "";
	case ValueType::T_OBJ:
		return "JsonObject";
	default:
		return "";
...
{
	this->m_is_array = value.m_is_array;
	this->m_members = std::move(value.m_members);
}
...
{
	m_is_array = true;
	std::map<std::string, JsonValue>::iterator it = m_members.find(std::to_string(index));
	if (it == m_members.end())
	{
		return m_members[std::to_string(index)] = JsonValue();
	}
	else
	{
		return it->second;
...
{
	std::map<std::string, JsonValue>::const_iterator it = m_members.find(std::to_string(index));
	if (it == m_members.end())
	{
		return JsonValue();
	}
...
bool JsonObject::read_string(const std::string &str, JsonValue &result, uint32_t &startPos)
{
	...
	while (true)
	{
		...
		else
		{
			...
			result.reset();
			result.type = ValueType::T_STRING;
			result.value.str = new std::string(std::move(value));
...
bool JsonObject::read_number(const std::string &str, JsonValue &result, uint32_t &startPos)
{
	...
	if (isFloat)
	{
		result.type = ValueType::T_DOUBLE;
		result.value.double_v = std::stod(floatStr);
	}
	else
	{
		result.type = ValueType::T_INT64;
		result.value.int64_v = (int64_t)std::stoll(floatStr);
	}
...
bool JsonObject::read_object(const std::string &str, uint32_t &startPos)
{
	...
	while (startPos < str.length())
	{
		...
		if (key.type != ValueType::T_STRING)
		{
			return false;
		}
		...
		m_members[*(key.value.str)] = std::move(value);
...
bool JsonObject::read_array(const std::string &str, JsonValue &result, uint32_t &startPos)
{
	...
	while (startPos < str.length())
	{
		...
		result.value.object->push(std::move(jsonValue));
...
void JsonObject::push(JsonValue&& value)
{
	...
	m_members[std::to_string(size)] = std::move(value);
}
